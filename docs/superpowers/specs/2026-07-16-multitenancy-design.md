...
> 但它是 ~800 行,不是 2500 —— P1/P2/P3/P5 都已各自绿过。**不要试图把 2500 行一次性写完。**
...
- **`profile/routes.rs:286`** 上传头像时硬写 `tenant_id: Uuid::nil()` —— 这是第 5 处 nil。content 收编后 nil 租户不再合法。
  **决定**:头像 content 落一个众所周知的 `PLATFORM_TENANT`(复用 §3.2 那个 demo 租户 id),**不是** claim 里的 tenant —— 因为头像是全局身份的一部分,与 `display_name` 同级、跟人走。
