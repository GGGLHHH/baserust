...
- **多进程拓扑**:开发单体 `Both`(一个进程挂全);生产 app / idm 分进程 + nginx 按 `/api/v1/auth` 前缀分流。`Mount` 枚举驱动挂哪些模块 + 连哪个库。
...
This project is indexed by GitNexus as **baserust** (1010 symbols, 2080 relationships, 85 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
