...
cp seed.prod.toml.example "$PROD_DIR/" # 模板;操作员在服务器 cp 成 seed.prod.toml 填强密码(真密码不打包)
...
echo "  ✓ docker-compose.prod.yml · .env · nginx.conf · seed.prod.toml.example · keys/ · scripts/initdb · import-images.sh"
echo ""
...
echo "  2. cp prod/seed.prod.toml.example prod/seed.prod.toml,填强超管密码(idm 启动幂等建号)"
echo "  3. scp -r $PROD_DIR user@server:/opt/baserust"
echo "  4. 服务器上:cd /opt/baserust/prod && ./import-images.sh"
echo ""
echo "⚠️ prod/keys/prod-ed25519.pem 是私钥,scp 用加密通道。seed.prod.toml 含明文密码,同样别泄露。"
